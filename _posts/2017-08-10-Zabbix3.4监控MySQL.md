...
```  
添加```/etc/zabbix/script/mysql/chk_mysql.sh```脚本  
```shell  
...
mysqladmin: [Warning] Using a password on the command line interface can be insecure.
 246645017  
```  
报错[Warning] Using a password on the command line interface can be insecure。谷歌资料找到一方法：将脚本```chk_mysql.sh```中mysqladmin的警告信息重定向到/dev/null，忽略掉告警信息。在原有命令中加上```2>/dev/null``` 就行了。  
```shell  
[root@ppp-2-85-103-23 mysql]# cat chk_mysql.sh 
#!/bin/bash
