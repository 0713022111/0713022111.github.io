...
![Alt text](http://p92ijvt1x.bkt.clouddn.com/Juniper_srx100_1.png "用串口线连接设备console口")  
```shell  
[edit]
...
2、在Policy--Policy Elements --Addresses--List把指定员工IP地址加入(Trust)（此处source-address为any）
3、将指定的网站加入组(Untrust)
117.79.235.137
117.79.235.138
117.79.235.139
...
【注】
注意策略的顺序
把员工访问公网any的策略设置成deny，然后把该策略放到访问指定网站策略的后面；即默认的policy放置到policy 1后面。  
